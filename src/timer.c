...
void TIM2_PWM_Init(void)
{
	...
	sMasterConfig.MasterOutputTrigger = TIM_MASTERSLAVEMODE_DISABLE;
...
void TIM2ch2_PWM_SetFrqDc(uint32_t freq, uint32_t dc)
{
	...
	sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
	sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
	sConfigOC.Pulse = pulse; //5400 = 50% duty cycle - range 0-0xffff*/
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	{
		TIM2ch2_PWM_SetFrqDc(10000, HAL_ADC_GetValue(&AdcHandle));
		TIM2ch2_PWM_Start();
	}
	}
